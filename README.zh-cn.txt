...
- 除了 /setCTByGroupCreator，也可以指定为 bot，请参阅「第四方 Bot 管理模式」节。
...
使用内联键盘编辑指定成员/全体成员的配置
在消息被发送后一段时间以后，内联键盘可能会失效

这种方式比 /grantCT 可能更加易用。

对已存在 CT 配置的用户使用 /editCT 时，内联键盘会反应其当前配置，并包含一个
Revoke 按钮，使用此按钮等效于对该用户运行 /revokeCT。
...
如果该用户存在 CT 配置，Bot 会发送提示消息，询问是否应该同时撤回其 CT 配额。

内联键盘上将包含两个按钮。
Revoke CT 按钮：按下等价于对该用户运行 /revokeCT
Retain CT 按钮：不执行任何操作。并删除此提示消息。
...
这里的第四方 Bot 指在当前群组中使用的除本 Bot 以外的 Bot。

第四方 Bot 管理模式，允许创建者和管理员使用本 Bot 管理其他 Bot 的管理权限和
头衔。以解决多个管理链上对第四方 Bot 的管理不便的问题。

此功能默认不会启用，必须由创建者主动启用才可以使用。

启用此功能
----------

要启用此功能，可由创建者使用以下任意方式

- /grantCT bot <perm list>
- /editCT bot，然后勾选权限列表并 “Submit”

要禁用此功能，可使用 /revokeCT bot 或 /editCT bot 并点击 Revoke 按钮

这个权限列表定义了第四方 Bot 的「权限边界集」

/setbotCT NEW_TITLE <perm list>
-------------------------------

必须回复某一第四方 Bot 的消息以指定 Bot。

NEW_TITLE 设定 Bot 的头衔

<perm list> 设定「权限期望集」，格式与 /grantCT 的格式相同。

第四方 Bot 将获得的权限为：

「权限边界集」- 执行命令的管理员当前不拥有的权限* - 不在「权限期望集」中的权限

*：不适用于当创建者执行此命令时

只能由创建者和管理员使用。且必须由创建者启用第四方 Bot 管理模式。
